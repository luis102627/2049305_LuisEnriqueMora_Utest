...
public class EnterInformation implements Task {
    ...
    public EnterInformation(List<PersonalInformation>data){
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(Enter.theValue(firstName).into(RegistrationForm.TXT_FIRSTNAME),
                ...
                Click.on(RegistrationForm.CHECK_TERM_OF_USE),
                Click.on(RegistrationForm.CHECK_PRIVACY_SETTINGS),
                Click.on(RegistrationForm.BTN_NEXT_LOCATION),
                WaitUntil.the(RegistrationForm.SUCCESSFUL_ACCOUNT_MESSAGE, isVisible()).forNoMoreThan(120).seconds());
    }

    public static EnterInformation toCreateAccount(List<PersonalInformation>data){
        return instrumented(EnterInformation.class, data);
    }
}
